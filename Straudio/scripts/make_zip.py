...
def  main():
  ...
  installer = "\installer\Straudio Installer.exe"
   
  if demo:
    installer = "\installer\Straudio Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Straudio manual.pdf" 
  ]
  ...
  ZIPNAME = "Straudio-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Straudio-v" + FULLVERSIONSTR + "-win-demo.zip"
